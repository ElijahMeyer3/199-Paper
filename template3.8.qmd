...
(Code to creat "second batch of AEs" if needed?)
...
Much like AEs, the instructor or head TA will create and clone a lab repository to all students that has the assessment and any other intangibles (like a data set) in it by changing the `assignment` object to contain the name of original lab repository. In the lab repository, we choose to create a starter document with places for students to write code under specific question numbers. The actual lab assessment that this document corresponds to can be found on the class website that is reference during lab time.
...
We can use these new data and change the code found in (??) to create repositories for each team.
...
The new expectation is that once groups are formed, one lab assessment will be turned in for each group instead of each individual. This further helps lesson the grading responsibilities to those that are assigned it.
